...
class ActorCrtic(object):
    ...
    def set_action_std(self, new_action_std):
        self.action_var = torch.full((self.num_actions,), new_action_std * new_action_std).to(device)
...
class PPOagent:
    ...
    def __init__(self, env, hidden_sizes=params['hidden_sizes'], \
        ...
        #Networks
        self.policy = ActorCrtic(hidden_sizes, self.num_actions, self.action_std)
        self.old_policy = ActorCrtic(hidden_sizes, self.num_actions, self.action_std)
    ...
    def update(self):
        ...
        rewards = (rewards - rewards.mean()) / (rewards.std() + 1e-7) # Revisar esta linea
    ...
    def load(self, path):
        self.policy.critic.load_state_dict(torch.load(path + "/critic.pth", map_location=device))
        ...
        self.policy.actor.load_state_dict(torch.load(path + "/actor.pth",  map_location=device))
