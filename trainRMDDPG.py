...
from params import PARAMS_TRAIN_RMDDPG, STATE_NAMES, ACTION_NAMES, REWARD_NAMES


BATCH_SIZE = PARAMS_TRAIN_RMDDPG['BATCH_SIZE']
EPISODES = PARAMS_TRAIN_RMDDPG['EPISODES']
n = PARAMS_TRAIN_RMDDPG['n']
TAU = 2 * np.pi  # No es el tau del agente
SHOW = PARAMS_TRAIN_RMDDPG['SHOW']
...
def main(path, params_ddpg):
    ...
    if PARAMS_TRAIN_RMDDPG['behavior_policy'] == 'gps':
        import torch
        ...
        behavior_path = PARAMS_TRAIN_RMDDPG['behavior_path']
        behavior_policy.load(behavior_path)
        weights = WEIGHTS
    elif PARAMS_TRAIN_RMDDPG['behavior_policy'] == 'ilqr':
        path2file = PARAMS_TRAIN_RMDDPG['behavior_path'].split('/')[-2] + '/'
        file_name = PARAMS_TRAIN_RMDDPG['behavior_path'].split('/')[-1]
    ...
    if not SHOW:
        create_report(path, method='rmddpg')
...
if __name__ == "__main__":
    PATH = 'results_rmddpg/' + date_as_path() + '/'
