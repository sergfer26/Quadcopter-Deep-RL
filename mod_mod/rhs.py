...
class Psi_rhs(StateRHS):
    ...
    def RHS(self, Dt):
        ...
        phi_ = self.V('phi')
        theta_ = self.V('theta')
        dpsi_ = (q_ * sin(phi_) + r_ * cos(phi_)) * (1 / cos(theta_))
        return dpsi_
...
class Q_rhs(StateRHS):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.SetSymbTimeUnits(mt)  # minuts
        vars = ['L', 'B', 'Iyy', 'w3', 'w1', 'p', 'r', 'Ixx', 'Izz', 'q']
        for name in vars:
            CONSTANTS[name].addvar_rhs(self)
